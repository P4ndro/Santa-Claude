...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    ...
    setErrors({});
    try {
      const data = await register(email, password, role, companyName);
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        ...
        <div className="bg-black rounded-2xl shadow-2xl w-full max-w-md p-8 md:p-10 border border-white">
          ...
          <div className="space-y-5">
            ...
            <div>
              <input
                ...
                className={`w-full px-4 py-3 bg-white border ${
                  errors.username ? 'border-white' : 'border-white'
            ...
            {/* Role Selection */}
            <div>
              <label className="block text-sm text-white mb-2">I am a:</label>
            ...
            {role === 'company' && (
              <div>
                <input
                  ...
                  onKeyPress={handleKeyPress}
                  className={`w-full px-4 py-3 bg-white border ${
                    errors.companyName ? 'border-white' : 'border-white'
                  } rounded-lg focus:outline-none focus:ring-2 focus:ring-white transition-all text-black`}
                />
                {errors.companyName && (
                  <p className="text-white text-sm mt-1">{errors.companyName}</p>
                )}
              </div>
            )}
